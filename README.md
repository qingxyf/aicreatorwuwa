...
- 四段活动时间：投稿、盲选、公开投票、结果公示。运营可设置各阶段的起止时间与当前公开阶段；正式模式公开端及 Node API 只开放当前投稿/盲选/投票阶段，测试预览模式同时显示全部流程，结果公示作为只读时间节点展示。
- 当前默认活动时间（北京时间）：投稿 9 月 1 日–10 月 8 日；盲选 10 月 9 日–10 月 12 日；公开投票 10 月 13 日–10 月 18 日；结果公示 10 月 19 日–10 月 21 日。
- 盲选：每账号每赛道 3 票，每次从两件作品中选 1 件，低曝光作品优先。每一次选择都由服务器签发一次性票据，不能由浏览器伪造对比作品。
- 投票：每账号每日每赛道 3 票，同一作品不可重复；作品展廊展示作者头像、昵称与票数。
- 运营后台：通过 ECS 服务端密码会话保护；可审阅投稿、作者、头像、阶段票数、初选胜场/曝光、决赛票数，并设置入围、公开展示和活动流程。公开端不含后台入口。
...
- 公开写操作需经过身份桥接核验，运营写操作需经过独立密码会话；盲选使用一次性票据，投票/投稿/阶段均在服务端校验。
- Node API 对写接口施加按账号与路由的固定窗口限流，并限制 JSON 与媒体请求体大小；媒体响应使用 `nosniff`，生产 CORS 仅允许配置的 Toy 页面 Origin。
- 应用层控制不能替代云安全组、ECS 防火墙和 OSS 对象级私有 ACL；当前 Bucket 按运营要求保持公共读，但活动上传对象必须为 private，公开访问统一经后端媒体路由授权。
...
首次部署时，Docker Compose 会自动按顺序执行 `server/migrations/*.sql` 初始化或升级 PostgreSQL。随后由白名单运营账号在 `/ops` 填写四段活动时间和当前阶段。
