...
面向 Bilibili Toy 的两个赛道活动站：用户只会看到规则、投稿和投票；白名单运营人员才可通过独立 `ops.html` 入口管理作品（自有静态服务器也兼容 `/ops`）。
...
首次部署时，Docker Compose 会自动执行 `server/migrations/001_init.sql` 初始化 PostgreSQL。随后由白名单运营账号在 `/ops` 填写三个阶段的时间和当前阶段。
